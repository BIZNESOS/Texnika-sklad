...
/* Общий пароль админ-панели — используется и в чате AI ("admin"), и в admin.html */
const ADMIN_PASSWORD = 'admin123';

/* Товары с учётом изменений из админки (localStorage), доступно всему сайту */
function allProducts() { return LS.get('ts_admin_products', PRODUCTS); }

/* Категории с учётом кастомных, созданных в админке */
function activeCategories() { return CATEGORIES.concat(LS.get('ts_custom_categories', [])); }
function categoryPhoto(id) { return (LS.get('ts_category_photos', {}) || {})[id] || null; }

/* ---------- Сжатие фото на клиенте (canvas), до загрузки в localStorage ---------- */
...
/* ---------- Отправка заказа в Telegram-бот (если подключён в настройках) ---------- */
function notifyTelegram(order) {
  const integ = LS.get('ts_integrations', {});
  if (!integ.tgToken || !integ.tgChatId) return;
  const itemsText = order.items.map(i => `• ${i.qty}× ${i.name} — ${money(i.price * i.qty)}`).join('\n');
  const text = `🛒 <b>Новый заказ #${order.id}</b>\n\n` +
    `Клиент: ${order.name}\nТелефон: ${order.phone}\n` +
    `Город: ${order.city}${order.address ? ', ' + order.address : ''}\n` +
    `Получение: ${order.delivery === 'delivery' ? 'Доставка' : 'Самовывоз'}\n` +
    `Оплата: ${{cash:'Наличными',transfer:'Перевод',other:'Другое'}[order.payment] || order.payment}\n` +
    (order.comment ? `Комментарий: ${order.comment}\n` : '') +
    `\n${itemsText}\n\n<b>Итого: ${money(order.total)}</b>`;
  fetch(`https://api.telegram.org/bot${integ.tgToken}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: integ.tgChatId, text, parse_mode: 'HTML' }),
  }).catch(() => {});
}
...
const state = {
  ...
  activeTab: 'hits',
  search: '',
  aiAdminStep: null, // null | 'awaiting_password'
};
...
function productArtSVG(p, size = 64) {
  if (p.photos && p.photos.length) {
    return `<img src="${p.photos[0]}" class="w-full h-full object-cover" alt="${p.name}">`;
  }
...
function renderCategories(targetSel) {
  const el = $(targetSel);
  if (!el) return;
  el.innerHTML = activeCategories().map(c => {
    const accent = (ART[c.id] && ART[c.id][1]) || 'var(--blue-500)';
    const photo = categoryPhoto(c.id);
    const art = photo ? `<img src="${photo}" class="w-full h-full object-cover rounded-[16px]" alt="${c.name}">`
      : (CATEGORY_ICON_SVG[c.id] ? CATEGORY_ICON_SVG[c.id](c.id) : `<i data-lucide="${c.icon || 'shapes'}"></i>`);
    return `
    <div class="cat-card flex flex-col items-center gap-2.5 py-5 px-2 cursor-pointer" onclick="filterByCategory('${c.id}')">
      <div class="cat-icon-wrap" style="color:${accent}">${art}</div>
  ...
  <div class="overlay-scrim fixed inset-0 z-[70] flex items-end sm:items-center justify-center" onclick="if(event.target===this) closeProduct()">
    <div class="modal-card w-full sm:max-w-4xl max-h-[92vh] overflow-y-auto hide-scrollbar rounded-t-3xl sm:rounded-3xl">
      ...
      <div class="grid sm:grid-cols-2 gap-0">
        <div class="p-6">
          <div class="rounded-2xl overflow-hidden aspect-square mb-3">${p.photos && p.photos.length ? `<img id="pdMainImg" src="${p.photos[0]}" class="w-full h-full object-cover" alt="${p.name}">` : productArtSVG(p, 110)}</div>
          <div class="flex gap-2 flex-wrap">
            ${p.photos && p.photos.length
              ? p.photos.slice(0, 10).map(src => `<div class="w-14 h-14 rounded-lg overflow-hidden border border-[var(--line)] cursor-pointer" onclick="document.getElementById('pdMainImg').src='${src}'"><img src="${src}" class="w-full h-full object-cover"></div>`).join('')
...
function sendAIMessage(presetText) {
  ...
  setTimeout(() => {
    ...
    // ---------- admin login flow ----------
    if (state.aiAdminStep === 'awaiting_password') {
      state.aiAdminStep = null;
      if (text.trim() === ADMIN_PASSWORD) {
        sessionStorage.setItem('ts_admin_auth', '1');
        aiAddBot('Пароль верный ✅ Открываю панель администратора…' +
          `<div class="mt-2"><a href="admin.html" class="ai-chip inline-flex px-3 py-1.5 rounded-full text-xs" style="color:#0f172a;background:#eef3ff;border-color:#dbe6fb">Перейти в админ-панель →</a></div>`);
        setTimeout(() => { location.href = 'admin.html'; }, 900);
      } else {
        aiAddBot('Неверный пароль. Напишите «admin», чтобы попробовать снова.');
      }
      return;
    }
    if (lower === 'admin' || lower === 'админ') {
      state.aiAdminStep = 'awaiting_password';
      aiAddBot('Это вход в панель администратора. Введите пароль:');
      return;
    }
